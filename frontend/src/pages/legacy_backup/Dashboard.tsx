...
const Dashboard: React.FC = () => {
  ...
  // 深度阅读
  const handleDeepRead = (cardId: number) => {
  ...
  const handleToggleFavorite = async (cardId: number) => {
    ...
    if (newFavorites.has(cardId)) {
      ...
      message.success(t('dashboard.unfavoriteSuccess') || '已取消收藏')
    } else {
      newFavorites.add(cardId)
      message.success(t('dashboard.favoriteSuccess') || '已收藏')
    }
  ...
  return (
    <div>
      ...
      <QuickViewModal
        ...
        isFavorite={selectedCardId ? favorites.has(selectedCardId) : false}
