...
class BehaviorTrackingService {
   ...
   * 记录用户行为（通用方法）
   */
  async logBehavior(behavior: Partial<BehaviorLog>): Promise<void> {
    ...
    try {
      const fullBehavior: BehaviorLog = {
        user_id: userId,
   ...
   * 记录卡片点击
   */
  async logCardClick(cardId: number): Promise<void> {
   ...
   * 记录卡片浏览（停留时长）
   */
  async logCardView(cardId: number, duration: number): Promise<void> {
    // 只记录停留超过3秒的浏览
    if (duration >= 3) {
      await this.logBehavior({
   ...
   * 记录收藏操作
   */
  async logFavorite(cardId: number): Promise<void> {
    await this.logBehavior({
   ...
   * 记录取消收藏
   */
  async logUnfavorite(cardId: number): Promise<void> {
    await this.logBehavior({
   ...
   * 记录搜索行为
   */
  async logSearch(query: string, mode: string = 'keyword'): Promise<void> {
    await this.logBehavior({
   ...
   * 记录分享操作
   */
  async logShare(cardId: number, platform?: string): Promise<void> {
    await this.logBehavior({
  ...
  async createSearchHistory(
    ...
    const userId = this.getUserId()
    if (!userId || !this.enabled) return

    try {
      const searchHistory: SearchHistoryCreate = {
        user_id: userId,
        query,
        mode,
   ...
   * 获取用户行为统计
   */
  async getUserStats(days: number = 30): Promise<BehaviorStats | null> {
    const userId = this.getUserId()
