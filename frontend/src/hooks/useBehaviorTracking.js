 ...
 */
export function useCardViewTracking(cardId, enabled = true) {
...
export function useSearchTracking() {
    const logSearch = useCallback((query, mode = 'keyword') => {
        behaviorTrackingService.logSearch(query, mode);
    }, []);
    const createSearchHistory = useCallback((query, mode, resultsCount, intent) => {
 ...
 * @param days 统计天数
 */
export function useUserBehaviorStats(days = 30) {
